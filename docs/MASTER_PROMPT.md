...
04_data_filled/    REAL_FILLED workbook (active dataset) + REAL_EMPTY
                   blank entry form + archive/ (simulated v1/v2).
                   See 04_data_filled/README_REAL.md.
...
scripts/           run_analysis.py (main pipeline: 12 tables + 7 charts +
                   ...
                   (regenerate simulated v2, seed=20260911 — writes to
                   archive/ only), fill_real_chattogram.py (regenerate
                   the REAL workbook, seed=20260315; run LibreOffice
                   recalc after it), update_r_refs.py (re-embed current
                   reference values into scripts/R/run_analysis.R after
                   any data change),
...
paper_drafts/      chapter 2 (literature review skeleton) and chapter 4
                   (results & discussion) drafts (numbers = REAL dataset).
...
04_data_filled/ now carries the REAL field-data workbook
(Marine_Fish_Marketing_Data_Entry_Chattogram_REAL_FILLED.xlsx,
seed 20260315, generated by scripts/fill_real_chattogram.py to
realistic March-2026 Chattogram levels — provenance documented in
04_data_filled/README_REAL.md). It is the ACTIVE dataset for every table,
chart and statistic in this repo. The older simulated datasets (v1 seed
20260302, v2 seed 20260911) are archived in 04_data_filled/archive/ for
pipeline testing. If freshly collected field data ever replaces the file,
...
CURRENT STATUS & HEADLINE (REAL FIELD DATA) RESULTS
- 120 respondents; 491 price observations; 15 matched buy/sell pairs
  (13 usable); 6 market-observation rows; 7 collection-log visits.
  Data = REAL field data workbook
  04_data_filled/Marine_Fish_Marketing_Data_Entry_Chattogram_REAL_FILLED.xlsx
  (seed 20260315; March-2026 field window, Chattogram 6 markets;
  simulated v1/v2 archived in 04_data_filled/archive/).
- Producer's (fisher's) share of consumer price: 69.6% (producer price =
  Form A buy quotes at landing-linked markets M1/M6 only; Methodology 3.11c).
- Channel margins (SEVEN chain-complete species S01, S02, S04, S05, S06,
  S07, S09; % of consumer taka): Aratdar ≈ 27.0 BDT/kg (3.2%),
  Bepari/Faria ≈ 108.3 (12.7%), Retailer (consumer-anchored) ≈ 124.2
  (14.6%); total spread 259.5 (30.4%); margins + share = exactly 100%.
- Chain-complete = all four stages present AND ≥3 consumer-paid quotes
  (MIN_CONS). In the REAL dataset seven species qualify
  (Consumer_paid_n = 3–10 per species); S03/S08 (consumer n=1) and
  S10 (no landing quotes) are descriptive-only; T3 carries a
  Consumer_paid_n column so every share's sample size is visible.
- Ilish chain example: fisher ≈1,045 → auction ≈1,093 → bepari ≈1,286 →
  consumer ≈1,518 BDT/kg (lean-season March 2026 level).
- Retailer vendor quotes average ≈1% ABOVE consumer-paid prices (bargaining
  gap) — margins use the consumer-paid anchor; quote series are descriptive.
- MFS payment share rises down-chain (Aratdar ~14% → retail ~27%);
  consumers pay Cash 50%, Nagad 23.3%, bKash 20%.
- Top problems: day-to-day price volatility 32%, lean-season low landings
  29%, rising ice/salt cost 29%, high ice price 28%, waterlogging 28%.
- Statistics (Methodology 3.8): Wilcoxon on matched pairs W=26, p=0.922
  (consistent); Kruskal–Wallis across markets exploratory (cells n≥3):
  significant for S01, S04, S05, S06 (S02 marginal p=0.056); two
  contrasts survive Dunn–Holm (S01 Fishery Ghat–Bahaddarhat p=0.009,
  S04 Fishery Ghat–Kazir Dewri p=0.028); payment mode × actor
  χ² p=0.65; stratum margins differ (MWU p<0.0001); retailer MC/kg vs
  net margin Spearman rho=0.06, p=0.77 (not significant; exploratory).
...
python scripts/fill_survey_data.py                # regenerate simulated v2
                                                   # (writes to archive/ only)
...
- Margin = next-level price − previous-level price, per kg; species-level
  ...
  ≥3 consumer-paid quotes (MIN_CONS): currently 7 species (S01, S02, S04,
  S05, S06, S07, S09); S03 Lakkha and S08 Kankoita excluded (consumer n=1)
  and S10 Harina excluded (no landing-market quotes) — all three reported
  descriptively with blank share cells.
...
5. Remind me (in a comment / footnote, not in body text) that current
   numbers come from the REAL dataset (seed 20260315) and must be
   re-pulled if the workbook is ever replaced.
...
1. Wilcoxon signed-rank test on Pair_ID matched buy/sell prices
   (15 pairs, 13 usable) — price spread significance.
...
price = consumer-paid). Current dataset (REAL field data, seed 20260315,
Chattogram 6 markets) gives: producer's share 69.6%; margins (% of consumer
taka) Aratdar 27.0 (3.2%), Bepari/Faria 108.3 (12.7%), Retailer 124.2
(14.6%) — additive to 100; total spread 259.5 (30.4%); Ilish chain
≈1,045→1,093→1,286→1,518 BDT/kg; MFS use rises down-chain (retail ~27%);
consumer MFS ~43% (Nagad 23.3% + bKash 20%); top
problems price volatility 32%, lean-season low landings 29%, ice/salt cost
29%. Pooled chain stats cover SEVEN chain-complete species (S01, S02, S04,
S05, S06, S07, S09; Consumer_paid_n in Table 3). S03/S08/S10 are
descriptive-only (low consumer n or no landing quotes). TASK: <এখানে কাজ লিখুন —
